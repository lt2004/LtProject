...
@interface NBZStreamingScrollLayout ()
/// The delegate will point to collection view's delegate automatically.
@property (nonatomic, weak) id <NBZStreamingCollectionViewDelegateLayout> delegate;
...
@implementation NBZStreamingScrollLayout
...
- (void)prepareLayout {
    ...
    NSAssert([self.delegate conformsToProtocol:@protocol(NBZStreamingCollectionViewDelegateLayout)], @"UICollectionView's delegate should conform to NBZStreamingCollectionViewDelegateLayout protocol");
    ...
    for (NSInteger section = 0; section < numberOfSections; ++section) {
        ...
        for (idx = 0; idx < itemCount; idx++) {
            ...
            attributes.frame = CGRectMake(xOffset, yOffset, itemWidth + 34, itemHeight);
