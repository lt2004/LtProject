...
- (instancetype)init {
    if (self = [super init]) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(appwillResignClearActive) name:@"StopAVPlayerAboutStatus" object:nil];
...
//    self.isDestory = YES;
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"StopAVPlayerAboutStatus" object:nil];
...
//        DDLogInfo(@"UIDeviceOrientationLandscapeLeft");
        [self orientationLeftFullScreen];
    }else if (orientation == UIDeviceOrientationLandscapeRight) {
//        DDLogInfo(@"UIDeviceOrientationLandscapeRight");
        [self orientationRightFullScreen];
    }else if (orientation == UIDeviceOrientationPortrait) {
//        DDLogInfo(@"UIDeviceOrientationPortrait");
        [self smallScreen];
    }
}
...
- (void)smallScreen {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        self.activityIndicatorView.center = CGPointMake(self.playerOriginalFrame.size.width / 2, self.playerOriginalFrame.size.height / 2);
        [self updateConstraintsIfNeeded];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"SmallScreen" object:nil];
    }];
...
- (void)appwillResignActive:(NSNotification *)note {
    ...
    if (self.player.rate == 1.0f) {
        // 当播放情况下，有电话或者切入后台暂停视频
       [self playOrPause:self.playOrPauseBtn];
    }
    
}

- (void)appwillResignClearActive {
    if (self.player.rate == 1.0f) {
        // 当播放情况下，有电话或者切入后台暂停视频
        [self playOrPause:self.playOrPauseBtn];
    }
...
////        DDLogInfo(@"totalBuffer：%.2f",totalBuffer);
//
//        //loading animation
...
- (void)dealloc {
    ...
    DDLogInfo(@"video player - dealloc");
}
