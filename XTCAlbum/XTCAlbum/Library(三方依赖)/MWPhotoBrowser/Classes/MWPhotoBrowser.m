...
@interface  MWPhotoBrowser() {
    UIImageView *_bgWaterImageView;
    UIButton *_insertImageDescButton;
    BOOL _isShowPhotoInfor; //  默认不展开照片信息
    UIInterfaceOrientation _showInterfaceOrientation; // 设备方向
}
@end
...
- (void)viewDidLoad {
    ...
    _imageDescTextView = [[UITextView alloc] init];
    _imageDescTextView.font = kSystemNormalFont;
    _imageDescTextView.textColor = [UIColor whiteColor];
    _imageDescTextView.backgroundColor = RGBACOLOR(0, 0, 0, 0.3);
    _imageDescTextView.editable = NO;
    _imageDescTextView.showsVerticalScrollIndicator = NO;
    [self.view addSubview:_imageDescTextView];
    
    
    if (self.isEdit) {
        UITapGestureRecognizer *tapGes = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(insertImageDescButton)];
        _imageDescTextView.userInteractionEnabled = YES;
        [_imageDescTextView addGestureRecognizer:tapGes];
        
        _insertImageDescButton = [UIButton buttonWithType:UIButtonTypeCustom];
        _insertImageDescButton.titleLabel.font = [UIFont systemFontOfSize:14];
        _insertImageDescButton.backgroundColor = RGBACOLOR(0, 0, 0, 0.3);
        _insertImageDescButton.layer.cornerRadius = 4;
        _insertImageDescButton.layer.masksToBounds = YES;
        [self.view addSubview:_insertImageDescButton];
        [_insertImageDescButton mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerX.equalTo(self.view);
            make.bottom.equalTo(self.view).with.offset(-15);
            make.size.mas_equalTo(CGSizeMake(100, 30));
        }];
        [_insertImageDescButton addTarget:self action:@selector(insertImageDescButton) forControlEvents:UIControlEventTouchUpInside];
        [self.view bringSubviewToFront:_insertImageDescButton];
        [self insertDescToTextView];
        
    } else {
        [self insertDescToTextView];
    }
    
    if (_publishSourceModelArray.count == 1 && _isEdit) {
        PublishSourceModel *sourceModel = _publishSourceModelArray[0];
        if (sourceModel.fileTypeEnum == PublishSourceFileVideoTypeEnum) {
            _insertImageDescButton.hidden = YES;
        } else {
    ...
    // 下滑模态消失
    UISwipeGestureRecognizer *swipeGesture = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(dismisButtonClick)];
...
- (void)handleDeviceOrientationDidChange:(UIInterfaceOrientation)interfaceOrientation{
    ...
    switch (device.orientation) {
        case UIDeviceOrientationLandscapeLeft: {
            _imageDescTextView.hidden = YES;
            _showInterfaceOrientation = UIInterfaceOrientationLandscapeLeft;
            UIButton *doneButton = _doneButton.customView;
            [doneButton setImage:nil forState:UIControlStateNormal];
            _bgWaterImageView.image = [MWPhotoBrowser image:[UIImage imageNamed:@"water_bg"] rotation:UIImageOrientationLeft];
//             _bgWaterImageView.frame = CGRectMake(0, 0, kScreenHeight, kScreenWidth);
        }
            break;
        case UIDeviceOrientationLandscapeRight: {
            _imageDescTextView.hidden = YES;
            _showInterfaceOrientation = UIInterfaceOrientationLandscapeRight;
            UIButton *doneButton = _doneButton.customView;
            [doneButton setImage:nil forState:UIControlStateNormal];
             _bgWaterImageView.image = [MWPhotoBrowser image:[UIImage imageNamed:@"water_bg"] rotation:UIImageOrientationRight];
//             _bgWaterImageView.frame = CGRectMake(0, 0, kScreenHeight, kScreenWidth);
        }
            break;
        case UIDeviceOrientationPortrait: {
            if (self.navigationController.navigationBarHidden) {
                ...
                _imageDescTextView.hidden = NO;
            }
            
            _showInterfaceOrientation = UIInterfaceOrientationPortrait;
            UIButton *doneButton = _doneButton.customView;
            if (_isLS) {
               [doneButton setImage:nil forState:UIControlStateNormal];
            } else {
                if (_isEdit) {
                     [doneButton setImage:[UIImage imageNamed:@"publish_delete"] forState:UIControlStateNormal];
                } else {
                     [doneButton setImage:[UIImage imageNamed:@"photo_infor"] forState:UIControlStateNormal];
                }
              
            }
            _bgWaterImageView.image = [UIImage imageNamed:@"water_bg"];
//            _bgWaterImageView.frame = CGRectMake(0, 0, kScreenWidth, kScreenHeight);
        }
            break;
        case UIDeviceOrientationPortraitUpsideDown: {
            _imageDescTextView.hidden = YES;
            _showInterfaceOrientation = UIInterfaceOrientationPortraitUpsideDown;
            _bgWaterImageView.image = [UIImage imageNamed:@"water_bg"];
//            _bgWaterImageView.frame = CGRectMake(0, 0, kScreenWidth, kScreenHeight);
        }
            break;
...
#pragma -mark 展示图片或视频描述
- (void)insertDescToTextView {
    PublishSourceModel *publishSourceModel = _publishSourceModelArray[_currentPageIndex];
    NSString *flagDesc = _isShowPhotoInfor ? [self loadPhotoInfor:publishSourceModel] : [self loadloadPhotoDesc:publishSourceModel];
    NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc] initWithString:flagDesc];
    ...
    [attributedString addAttribute:NSKernAttributeName value:@(2) range:NSMakeRange(0, attributedString.string.length)];
    ...
    [attributedString addAttribute:NSFontAttributeName value:kSystemNormalFont range:NSMakeRange(0, attributedString.string.length)];
    CGSize titleSize = [attributedString.string boundingRectWithSize:CGSizeMake(kScreenWidth, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont fontWithName:@"Helvetica" size:18]} context:nil].size;
    if (titleSize.height > 200) {
        [_imageDescTextView mas_remakeConstraints:^(MASConstraintMaker *make) {
        ...
        [_imageDescTextView mas_remakeConstraints:^(MASConstraintMaker *make) {
            ...
            if (attributedString.string && attributedString.string.length) {
    ...
    _imageDescTextView.attributedText = attributedString;
    if (_isEdit) {
        if (attributedString.string && attributedString.string.length) {
            _insertImageDescButton.hidden = YES;
        } else {
             _insertImageDescButton.hidden = NO;
        }
    } else {
...
- (NSString *)loadPhotoInfor:(PublishSourceModel *)sourceModel {
    NSString *flagStr = @"";
    if (sourceModel.make && sourceModel.make.length) {
        flagStr = [NSString stringWithFormat:@"%@品牌:%@\n", flagStr, sourceModel.make];
    }
    if (sourceModel.model && sourceModel.model.length) {
        flagStr = [NSString stringWithFormat:@"%@型号:%@\n", flagStr, sourceModel.model];
    }
    
    if (sourceModel.lensModel && sourceModel.lensModel.length) {
        flagStr = [NSString stringWithFormat:@"%@%@\n", flagStr, sourceModel.lensModel];
    }
    
    if (sourceModel.focalLength && sourceModel.focalLength.length) {
        flagStr = [NSString stringWithFormat:@"%@焦距:%@\n", flagStr, sourceModel.focalLength];
    }
    if (sourceModel.apertureFNumber && sourceModel.apertureFNumber.length) {
        flagStr = [NSString stringWithFormat:@"%@光圈:%@\n", flagStr, sourceModel.apertureFNumber];
    }
    if (sourceModel.exposureTime && sourceModel.exposureTime.length) {
        flagStr = [NSString stringWithFormat:@"%@曝光时间:%@\n", flagStr, sourceModel.exposureTime];
    }
    if (sourceModel.exposureBiasValue && sourceModel.exposureBiasValue.length) {
        flagStr = [NSString stringWithFormat:@"%@曝光补偿:%@\n", flagStr, sourceModel.exposureBiasValue];
    }
    if (sourceModel.exposureProgram && sourceModel.exposureProgram.length) {
        flagStr = [NSString stringWithFormat:@"%@曝光模式:%@\n", flagStr, sourceModel.exposureProgram];
    }
    if (sourceModel.ISOSpeedRatings && sourceModel.ISOSpeedRatings.length) {
        flagStr = [NSString stringWithFormat:@"%@ISO感光度:%@\n", flagStr, sourceModel.ISOSpeedRatings];
    }
    if (sourceModel.dateTimeOriginal && sourceModel.dateTimeOriginal.length) {
        flagStr = [NSString stringWithFormat:@"%@拍摄时间:%@\n", flagStr, sourceModel.dateTimeOriginal];
    }
    if (flagStr.length) {
...
- (NSString *)loadloadPhotoDesc:(PublishSourceModel *)sourceModel {
    return sourceModel.sourceDesc ? sourceModel.sourceDesc : @"";
}


#pragma mark - 添加图片描述
- (void)insertImageDescButton {
    // 所见发布即所得暂时不需要在预览照片时添加描述
    /*
    UIStoryboard *storyBoard = [UIStoryboard storyboardWithName:@"PublishInsertImageDesc" bundle:nil];
    PublishInsertImageDescViewController *publishInsertImageDescVC = [storyBoard instantiateViewControllerWithIdentifier:@"PublishInsertImageDescViewController"];
    publishInsertImageDescVC.imageDescCallback = ^(NSString *imageDesc) {
        // 有修改
        PublishSourceModel *flagSourceModel = self->_publishSourceModelArray[self->_currentPageIndex];
        flagSourceModel.sourceDesc = imageDesc;
        [self->_publishSourceModelArray replaceObjectAtIndex:self->_currentPageIndex withObject:flagSourceModel];
        NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc] initWithString:imageDesc];
        NSMutableParagraphStyle *paragraphStyle = [[NSMutableParagraphStyle alloc] init];
        [paragraphStyle setLineSpacing:7];
        [attributedString addAttribute:NSParagraphStyleAttributeName value:paragraphStyle range:NSMakeRange(0, attributedString.string.length)];
        [attributedString addAttribute:NSKernAttributeName value:@(2) range:NSMakeRange(0, attributedString.string.length)];
        [attributedString addAttribute:NSForegroundColorAttributeName value:[UIColor whiteColor] range:NSMakeRange(0, attributedString.string.length)];
        [attributedString addAttribute:NSFontAttributeName value:kSystemNormalFont range:NSMakeRange(0, attributedString.string.length)];
        self->_imageDescTextView.attributedText = attributedString;
        self->_insertImageDescButton.hidden = YES;
        CGSize titleSize = [imageDesc boundingRectWithSize:CGSizeMake(kScreenWidth, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont fontWithName:@"Helvetica" size:18]} context:nil].size;
//        CGSize titleSize = [imageDesc sizeWithFont:[UIFont fontWithName:@"Helvetica" size:18] constrainedToSize:CGSizeMake(kScreenWidth, MAXFLOAT) lineBreakMode:UILineBreakModeWordWrap];
        if (titleSize.height > 200) {
            [self->_imageDescTextView mas_remakeConstraints:^(MASConstraintMaker *make) {
                make.left.right.equalTo(self.view);
                if (kDevice_Is_iPhoneX) {
                    make.bottom.equalTo(self.view).with.offset(-kBottom_iPhoneX);
                } else {
            ...
            [self->_imageDescTextView mas_remakeConstraints:^(MASConstraintMaker *make) {
                ...
                if (self->_imageDescTextView.attributedText.string && self->_imageDescTextView.attributedText.string.length) {
                    make.height.mas_equalTo(titleSize.height+20);
                } else {
                    make.height.mas_equalTo(0);
                }
    ...
    // 有修改
    PublishSourceModel *sourceModel = _publishSourceModelArray[_currentPageIndex];
    NSString *flagString = sourceModel.sourceDesc;
    publishInsertImageDescVC.imageDesc = flagString;
    [self.navigationController pushViewController:publishInsertImageDescVC animated:YES];
     */
...
- (void)performLayout {
    ...
    if ([self.navigationController.viewControllers objectAtIndex:0] == self) {
        ...
        [deleteButton addTarget:self action:@selector(doneButtonPressed:) forControlEvents:UIControlEventTouchUpInside];
        deleteButton.frame = CGRectMake(0, 0, 30, 30);
        deleteButton.contentHorizontalAlignment = UIControlContentHorizontalAlignmentRight;
        _doneButton = [[UIBarButtonItem alloc] initWithCustomView:deleteButton];
        if (_isEdit) {
            [deleteButton setImage:[UIImage imageNamed:@"publish_delete"] forState:UIControlStateNormal];
        } else {
             if (self.isLS) {
                 [deleteButton setImage:nil forState:UIControlStateNormal];
             } else {
                 [deleteButton setImage:[UIImage imageNamed:@"photo_infor"] forState:UIControlStateNormal];
             }
            
        }
        self.navigationItem.rightBarButtonItem = _doneButton;
        
        UIBarButtonItem *backButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"new_back_"] style:UIBarButtonItemStylePlain target:self action:@selector(dismisButtonClick)];
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    [_imageDescTextView setContentOffset:CGPointZero animated:YES];
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
	// Hide controls when dragging begins
        if (_showInterfaceOrientation == UIInterfaceOrientationLandscapeLeft || _showInterfaceOrientation == UIInterfaceOrientationLandscapeRight) {
            [self setControlsHidden:YES animated:YES permanent:NO];
        } else {
            if (_statusBarShouldBeHidden == NO) {
...
- (void)setControlsHidden:(BOOL)hidden animated:(BOOL)animated permanent:(BOOL)permanent {
    ...
    if (hidden || _showInterfaceOrientation == UIInterfaceOrientationLandscapeLeft || _showInterfaceOrientation == UIInterfaceOrientationLandscapeRight) {
        _imageDescTextView.hidden = YES;
    } else {
        _imageDescTextView.hidden = NO;
    }
...
- (void)doneButtonPressed:(id)sender {
    if (_isEdit) {
        if (self.imageArrayDeleteCallback) {
            self.imageArrayDeleteCallback(_currentPageIndex);
        }
//        [self dismissViewControllerAnimated:YES completion:nil];
        [self dismisButtonClick];
    } else {
        if (_isLS) {
            ...
            _isShowPhotoInfor = !_isShowPhotoInfor;
            [self insertDescToTextView];
        }
...
- (void)dismisButtonClick {
    if (_showInterfaceOrientation == UIInterfaceOrientationPortrait) {
