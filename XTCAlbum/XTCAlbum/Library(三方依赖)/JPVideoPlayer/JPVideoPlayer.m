...
- (JPVideoPlayerModel *)playExistedVideoWithURL:(NSURL *)url
    ...
    if ([[UIDevice currentDevice] systemVersion].intValue >= 10){
        self.playerModel.player.automaticallyWaitsToMinimizeStalling = NO;
    }
    return model;
}
    ...
    if ([keyPath isEqualToString:@"status"]) {
        ...
        switch (status) {
            ...
            case AVPlayerItemStatusFailed:{
                [[JPVideoPlayerCache sharedCache] removeVideoCacheForKey:self.playerModel.url.absoluteString completion:^{
                    [self stopCheckBufferingTimerIfNeed];
...
- (void)playFinishVideoSeek {
    [[NSNotificationCenter defaultCenter] postNotificationName:@"XTCVideoPlayFinish" object:nil];
//    __weak typeof(self.playerModel) weak_Item = self.playerModel;
//    [self.playerModel.player seekToTime:CMTimeMake(0, 1) completionHandler:^(BOOL finished) {
//        __strong typeof(weak_Item) strong_Item = weak_Item;
//        if (!strong_Item) return;
