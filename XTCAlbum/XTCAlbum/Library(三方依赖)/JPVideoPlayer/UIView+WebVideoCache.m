...
@property (nonatomic, assign) BOOL videoLandscape;
...
#pragma mark - 横竖屏切换
- (void)jp_gotoLandscape:(BOOL)isLandscape byDeviceOrientation:(UIInterfaceOrientation) deviceOrientation {
    // isLandscape是否改为横屏
    self.helper.videoLandscape = isLandscape;
    [self jp_gotoLandscapeAnimated:YES byDeviceOrientation:deviceOrientation
                        completion:nil];
}

- (void)jp_gotoLandscapeAnimated:(BOOL)flag byDeviceOrientation:(UIInterfaceOrientation) deviceOrientation
    ...
    if (flag) {
        [UIView animateWithDuration:0.25
                         ...
                         animations:^{
                             [self executeLandscapeByDeviceOrientation:deviceOrientation];
                         }
                         completion:^(BOOL finished) {
                             ...
                             [[NSNotificationCenter defaultCenter] postNotificationName:kVideoStatusBarHide object:nil];
                         }];
    }
    else{
        [self executeLandscapeByDeviceOrientation:deviceOrientation];
    ...
    if (self.helper.videoLandscape) {
        /*
         if ([DeviceOrientation sharedDeviceOrientation].deviceDirection == TgDirectionleft) {
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kVideoStatusBarShow object:nil];
...
- (void)executeLandscapeByDeviceOrientation:(UIInterfaceOrientation) deviceOrientation {
    UIView *videoPlayerView = self.helper.videoPlayerView;
