...
- (void)landscapeButtonDidClick:(UIButton *)button {
    ...
    if (self.playerView.frame.size.width > self.playerView.frame.size.height) {
    ...
    if (isLandscape) {
        // 横屏
        if (self.playerView.jp_viewInterfaceOrientation == JPVideoPlayViewInterfaceOrientationPortrait) {
            [self.playerView jp_gotoLandscape:YES byDeviceOrientation:UIInterfaceOrientationLandscapeRight];
        } else {
            [self.playerView jp_gotoPortrait];
        }
    } else {
        // 竖屏
        if (self.playerView.jp_viewInterfaceOrientation == JPVideoPlayViewInterfaceOrientationPortrait) {
             [self.playerView jp_gotoLandscape:NO byDeviceOrientation:UIInterfaceOrientationLandscapeRight];
        } else {
            [self.playerView jp_gotoPortrait];
        }
    }
    /*
    self.playButton.jp_viewInterfaceOrientation == JPVideoPlayViewInterfaceOrientationPortrait ? [self.playerView jp_gotoLandscape:isLandscape byDeviceOrientation:UIInterfaceOrientationLandscapeRight] : [self.playerView jp_gotoPortrait];
    */
//    self.playerView.jp_viewInterfaceOrientation == JPVideoPlayViewInterfaceOrientationPortrait ? [self.playerView jp_gotoLandscape:isLandscape] : [self.playerView jp_gotoPortrait];
}
...
- (void)setFrame:(CGRect)frame {
    ...
    if (kDevice_Is_iPhoneX) {
        self.userInteractionContainerView.frame = CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height - kJPVideoPlayerControlBarHeight-kBottom_iPhoneX);
    } else {
...
- (void)_setup {
    ...
    if (self.needAutoHideControlViewWhenUserTapping) {
        ...
        // 竖屏全屏情况下，下滑最小化
        UISwipeGestureRecognizer *dismisSwipeGestureRecognizer = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(exitFullPLayView:)];
        dismisSwipeGestureRecognizer.direction = UISwipeGestureRecognizerDirectionDown;
        [self addGestureRecognizer:dismisSwipeGestureRecognizer];
...
#pragma mark - 退出全屏播放
- (void)exitFullPLayView:(UIPanGestureRecognizer *)sender  {
    NSArray *controlArray = [self.controlContainerView subviews];
    if (controlArray.count) {
        JPVideoPlayerControlView *videoPlayerControlView  = controlArray.firstObject;
        NSArray *barArray = videoPlayerControlView.subviews;
        if (barArray.count) {
            JPVideoPlayerControlBar *videoPlayerControlBar;
            for (UIView *subView in barArray) {
                if ([subView isKindOfClass:[JPVideoPlayerControlBar class]]) {
                    videoPlayerControlBar = (JPVideoPlayerControlBar *)subView;
                } else {
                    
                }
            }
            if (videoPlayerControlBar.landscapeButton.isSelected) {
                [videoPlayerControlBar landscapeButtonDidClick:videoPlayerControlBar.landscapeButton];
//                if (videoPlayerControlBar.playButton.isSelected) {
//                    [self jp_pause];
//                } else {
//
//                }
