...
@synthesize yb_hideShowToolViews = _yb_hideShowToolViews;
...
- (UIView *)yb_foregroundView {
    return self.imageScrollView.photoDetailImageView;
}
...
- (void)yb_orientationChangeAnimationWithExpectOrientation:(UIDeviceOrientation)orientation {
    [self updateImageLayoutWithOrientation:orientation previousImageSize:self.imageScrollView.photoDetailImageView.image.size];
}
...
- (void)updateImageLayoutWithOrientation:(UIDeviceOrientation)orientation previousImageSize:(CGSize)previousImageSize {
    ...
    UIImage *image = self.imageScrollView.photoDetailImageView.image;
...
//    CGFloat maxZoomScale = imageType == YBIBScrollImageTypeThumb ? 1 : [data.layout yb_maximumZoomScaleWithContainerSize:containerSize imageSize:imageSize orientation:orientation];
    ...
    // 滚动显示exif信息页
    if ([StaticCommonUtil app].portraitScreen) {
        self.imageScrollView.contentSize = CGSizeMake(kScreenWidth, kScreenHeight-(kScreenHeight-imageViewFrame.size.height)*0.5 +kScreenHeight-100);
    } else {
...
//    self.imageScrollView.maximumZoomScale = maxZoomScale;
    ...
    if (ABS(scale) <= 0.001) {
        self.imageScrollView.photoDetailImageView.frame = imageViewFrame;
    } else {
        [UIView animateWithDuration:0.25 animations:^{
            self.imageScrollView.photoDetailImageView.frame = imageViewFrame;
        }];
    }
    //    if (imageType == YBIBScrollImageTypeOriginal) {
    [self.imageScrollView addSubview:self.imageScrollView.inforTableView];
    self.imageScrollView.inforTableView.frame = CGRectMake(0, self.imageScrollView.contentSize.height-kScreenHeight+100, kScreenWidth, kScreenHeight-100);
    //    }
    self.imageScrollView.data = data;
    [self.imageScrollView loadSourceInforByImageData:data];
}
...
- (void)yb_imageData:(YBIBImageData *)data startLoadingWithStatus:(YBIBImageLoadingStatus)status {
    switch (status) {
        case YBIBImageLoadingStatusDecoding: {
            if (!self.imageScrollView.photoDetailImageView.image) {
...
- (void)yb_imageData:(YBIBImageData *)data readyForImage:(__kindof UIImage *)image {
    [self.yb_auxiliaryViewHandler() yb_hideLoadingWithContainer:self];
    if (self.imageScrollView.photoDetailImageView.image == image) return;
    
    CGSize size = self.imageScrollView.photoDetailImageView.image.size;
...
- (void)yb_imageData:(YBIBImageData *)data readyForThumbImage:(__kindof UIImage *)image {
    if (self.imageScrollView.photoDetailImageView.image) return;
...
- (void)respondsToTapSingle:(UITapGestureRecognizer *)tap {
    ...
    if (data.singleTouchBlock) {
        ...
        [self hideAuxiliaryView];
        // 隐藏工具栏
        self.yb_hideShowToolViews();
    }
}

- (void)respondsToTapDouble:(UITapGestureRecognizer *)tap {
    if (self.yb_isRotating()) return;
    if (self.imageScrollView.inforTableView.hidden == NO) {
        return;
    }
...
- (void)respondsToPan:(UIPanGestureRecognizer *)pan {
    if (self.yb_isRotating()) return;
    if ([StaticCommonUtil app].portraitScreen == NO) {
        return;
    }
    YBIBInteractionProfile *profile = ((YBIBImageData *)self.yb_cellData).interactionProfile;
    if (profile.disable) return;
    if ((CGRectIsEmpty(self.imageScrollView.photoDetailImageView.frame) || !self.imageScrollView.photoDetailImageView.image)) return;
...
- (void)scrollViewDidZoom:(UIScrollView *)scrollView {
    if (_imageScrollView.inforTableView.hidden) {
        YBIBImageData *data = self.yb_cellData;
        if (data.imageDidZoomBlock) {
            data.imageDidZoomBlock(data, scrollView);
        }
        
        CGRect imageViewFrame = self.imageScrollView.photoDetailImageView.frame;
    ...
    if (_imageScrollView.inforTableView.hidden) {
        if (scale == 1) {
            ...
            if ([StaticCommonUtil app].portraitScreen) {
                ...
                CGSize containerSize = self.yb_containerSize(UIDeviceOrientationLandscapeLeft);
                CGSize contentSize = [self contentSizeWithContainerSize:containerSize imageViewFrame:imageViewFrame];
                self.imageScrollView.contentSize = contentSize;
            }
        } else {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if (scrollView.zoomScale == 1) {
        if (scrollView.contentOffset.y > 10) {
            self.yb_hideToolViews(YES);
            self.imageScrollView.inforTableView.hidden = NO;
        } else {
            self.yb_hideToolViews(NO);
            self.imageScrollView.inforTableView.hidden = YES;
        }
    } else {
        self.imageScrollView.inforTableView.hidden = YES;
    }
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    if (decelerate) {
        AppDelegate *appDelegate = [StaticCommonUtil app];
        if (scrollView.contentOffset.y < -80 && scrollView.zoomScale == 1 && appDelegate.portraitScreen) {
            // 退出照片或视频的预览
            if (self.yb_isRotating()) return;
            [self hideTailoringImageView];
            [self hideAuxiliaryView];
        ...
        if (_imageScrollView.inforTableView.hidden == NO) {
            [self.imageScrollView setMaximumZoomScale:1];
        } else {
            [self.imageScrollView setMaximumZoomScale:2];
        }
        NSLog(@"滑动松手了");
    }
}
