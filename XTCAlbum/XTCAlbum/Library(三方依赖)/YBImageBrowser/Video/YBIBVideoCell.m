...
- (void)updateImageLayoutWithOrientation:(UIDeviceOrientation)orientation previousImageSize:(CGSize)previousImageSize {
    ...
    self.videoView.contentSize = CGSizeMake(kScreenWidth, kScreenHeight-(kScreenHeight-imageViewFrame.size.height)*0.5 + kScreenHeight-100);
    self.videoView.inforTableView.frame = CGRectMake(0, self.videoView.contentSize.height-kScreenHeight+100, kScreenWidth, kScreenHeight-100);
    self.videoView.delegate = self;
    
    self.videoView.data = data;
    [self.videoView loadSourceInforByVideoData:data];
}

- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if (self.videoView.playing) {
        self.yb_hideToolViews(YES);
    } else {
        if (scrollView.contentOffset.y > 10) {
            self.yb_hideToolViews(YES);
            self.videoView.inforTableView.hidden = NO;
        } else {
            self.yb_hideToolViews(NO);
            self.videoView.inforTableView.hidden = YES;
        }
    }
...
- (void)yb_videoData:(YBIBVideoData *)data readyForThumbImage:(UIImage *)image {
    if (!self.videoView.thumbImageView.image) {
        ...
        [self.videoView.thumbImageView.layer addAnimation:transition forKey:nil];
...
- (void)yb_preparePlayForVideoView:(YBIBVideoView *)view {
    self.videoView.scrollEnabled = NO;
...
- (void)yb_startPlayForVideoView:(YBIBVideoView *)view {
    [self.videoView setContentOffset:CGPointMake(0, 0) animated:YES];
    self.videoView.inforTableView.hidden = YES;
    self.videoView.scrollEnabled = NO;
...
- (void)yb_finishPlayForVideoView:(YBIBVideoView *)view {
    [self.yb_backView ybib_videoPlayingRemove:self];
//    [self hideToolViews:NO];
    self.videoView.scrollEnabled = YES;
}
