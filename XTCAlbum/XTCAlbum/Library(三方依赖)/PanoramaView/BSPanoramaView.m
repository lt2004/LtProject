...
- (void)setImageWithName:(UIImage *)flagImage {
    ...
    [loader textureWithCGImage:textureImage.CGImage options:options queue:dispatch_get_main_queue() completionHandler:^(GLKTextureInfo * _Nullable textureInfo, NSError * _Nullable outError) {
        [[PanoramaManager sharedInstance] registerView:weakSelf];
        if (weakSelf.shouldUnload) {  // 因为是异步加载，所以需要考虑调用完 unloadImage 才加载成功的情况
            [[PanoramaManager sharedInstance] unRegisterView:self];
            GLuint name = textureInfo.name;
            glDeleteTextures(1, &name);
            glDrawElements(GL_TRIANGLES, weakSelf.numIndices, GL_UNSIGNED_SHORT, 0);
            return ;
        }
        
        /// 设置着色器的纹理
        weakSelf.effect = [[GLKBaseEffect alloc] init];
        weakSelf.effect.texture2d0.enabled = GL_TRUE;
        weakSelf.effect.texture2d0.name = textureInfo.name;
    }];
}
...
- (void)drawRect:(CGRect)rect {
    if ([[StaticCommonUtil app].topViewController isKindOfClass:[ProDetailShowViewController class]]) {
        [self update];
